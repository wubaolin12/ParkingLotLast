	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<c:forEach items="${ulist}" var="ul" varStatus="uu">
			<tr class="text-c">
				...
				<td>${ul.u_id}</td>
				<td><u style="cursor:pointer" class="text-primary" onclick="member_show('张三','member-show.html','10001','360','400')">${ul.u_name}</u></td>
				<!-- <td>男</td> -->
				<td>${ul.u_phone}</td>
				<td class="td-status"><span class="label label-success radius">${ul.param.pm_name}</span></td>
				<td class="td-manage">
				<c:if test="${ul.param.pm_name=='启用'}">
				<a style="text-decoration:none" onClick="member_stop(this,'${ul.u_id}','${pageel.currentpage}')" href="javascript:;" title="禁用"><i class="Hui-iconfont">&#xe631;</i></a> 
				</c:if>
				<c:if test="${ul.param.pm_name=='禁用'}">
				<a style="text-decoration:none" onClick="member_start(this,'${ul.u_id}','${pageel.currentpage}')" href="javascript:;" title="启用"><i class="Hui-iconfont">&#xe6e1;</i></a> 
				</c:if>
				<a title="重置密码" href="javascript:;" onclick="member_resetpwd(this,'${ul.u_id}')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a> 
				<a style="text-decoration:none" class="ml-5" href="<%=basePath %>emp/toUpadatePage.action?u_id=${ul.u_id}&u_name=${ul.u_name}&currentpage=${pageel.currentpage}" title="修改">修改</a>
				<a title="删除" href="javascript:;" onclick="member_del(this,'${ul.u_id}','${pageel.currentpage}')" class="ml-5" >删除</a>
				</td>
			</tr>
		</c:forEach>
		</tbody>
