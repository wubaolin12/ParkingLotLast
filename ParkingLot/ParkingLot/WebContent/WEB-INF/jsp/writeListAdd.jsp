		...
		<form action="" method="post" class="form form-horizontal"
			...
			<div class="row cl">
				<label class="form-label col-xs-4 col-sm-3"><span
					class="c-red">*</span>请输入车牌号：</label>
				<div class="formControls col-xs-8 col-sm-9">
					<input type="text" class="input-text" value="" placeholder=""
						id="platenumber" name="platenumber">
				</div>
			</div>
			...
			<div class="row cl">
				<div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-3">
					<input class="btn btn-primary radius" type="button"
						value="&nbsp;&nbsp;升级为白名单车辆&nbsp;&nbsp;" onclick="checkNum()">
				</div>
			</div>
		</form>
	</article>
...
<script type="text/javascript">
	function checkNum(){
		var carNum = document.getElementById("platenumber").value; 

		$.ajax({
			url:"${path}/writeList/checkCarNum.action",
			type: "post",
			dataType:"text",
			data:"carNum="+carNum,
			success:function(result){
				...
				if(result == 1){
					alert("["+carNum+"]成功升级为白名单会员！")
				}else{
					alert("["+carNum+"]资格不符，请检查车辆状态！")
				}
